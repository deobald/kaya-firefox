(function() {
  ...
  async function loadSettings() {
    try {
      ...
      if (result.server) {
        ...
        serverInput.value = 'https://kaya.town';
      }
      if (result.email) {
  ...
  async function saveSettings() {
    const server = serverInput.value.trim() || 'https://kaya.town';
  ...
  async function testConnection() {
    ...
    try {
      const response = await fetch(`${server}/api/v1/${encodeURIComponent(email)}/anga`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        showStatus('Authentication failed - check your email and password', 'error');
      } else {
